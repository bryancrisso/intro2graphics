...
public class Renderer {
	...
	protected ColorRGB trace(Scene scene, Ray ray, int bouncesLeft) {
        ...
        Vector3 O = ray.getOrigin();
     	...
     	// Direct illumination at this point
		...
		else
		{
			...
			//TODO calculate direction R of the reflected ray
			Vector3 R = ray.getDirection().scale(-1).reflectIn(N);
			//TODO spawn a reflected ray with bias
			Ray reflectedRay = new Ray(P.add(N.scale(EPSILON)), R);
			//TODO Calculate reflectedIllumination by tracing ray
	...
	private ColorRGB illuminate(Scene scene, SceneObject object, Vector3 P, Vector3 N, Vector3 O) {
		...
		for (int i = 0; i < pointLights.size(); i++) {
			...
			Vector3 R = L.subtract(N.scale(2 * N.dot(L))).normalised();
			...
			if (hit.getObjectHit() == null || hit.getDistance() > distanceToLight)
			{
